...
fn process<F>(p1 : &Point, p2: &Point, lights : &mut[bool; 1000000], action : F)
...
fn part1(input:&str) -> String{
    ...
    for order in orders.iter() {
        match order {
            Order::TurnOn(p1,p2) => {process(p1,p2, &mut lights, |_b| true)},
            Order::TurnOff(p1,p2) => {process(p1,p2, &mut lights, |_b| false)},
            Order::Toggle(p1,p2) => {process(p1,p2, &mut lights, |b| !b)},
        }
    }
    let light_count = lights.iter().filter(|b| **b).count();
